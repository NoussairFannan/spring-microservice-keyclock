    ...
    const fetchProduits = async () => {
        try {
            ...
            if (statusCode === 503 || err.code === 'ECONNREFUSED') {
                setError('Service Produit non disponible. Vérifiez que le service est démarré sur le port 8085.');
            } else if (statusCode === 401) {
                setError('Non autorisé. Veuillez vous reconnecter.');
            } else if (statusCode === 500) {
                setError('Erreur serveur. Vérifiez les logs du service.');
            } else {
                setError(`Erreur: ${errorMessage} (Status: ${statusCode || 'N/A'})`);
            }
        } finally {
            setLoading(false);
        }
